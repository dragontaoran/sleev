...
# portion the size of the B-spline basis according to the 
# size of two sex groups
...
pred_loglike.1 <- run.time.mins <- rep(NA, length(sns)) 
...
for (i in 1:length(sns)) {
  ...
  res.1 <- cv_linear2ph(Y="CD4_val_sq10", Y_unval="CD4_unval_sq10",
                        ...
                        verbose = FALSE)
  # save run time
  run.time.mins[i] <- difftime(Sys.time(), start.time, units = "mins")
...
```{r}
out <- data.frame(sns, pred_loglike.1, run.time.mins)
options(digits = 6)
...
colnames(Bspline) <- paste("bs", 1:sn, sep="")
data.sieve <- data.frame(cbind(mock.vccc, Bspline))
Bspline <- splines::bs(mock.vccc$VL_unval_l10, df=sn, degree=3, 
                       intercept=TRUE)
colnames(Bspline) <- paste("bs", 1:sn, sep="")
