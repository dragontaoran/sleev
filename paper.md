...
error-prone phase-one data and the validated phase-two data.
...
similar for the two functions and listed in Table 1. In addition to the arguments for error-prone and error-free
...
@giganti2020accounting. Table 2 describes the variables in this dataset.
...
We now illustrate how to obtain the SMLEs using the `sleev` package with the `mock.vccc` dataset. Specifically, we show how to fit a linear
...
error-prone, partially validated variables, whereas sex is error-free. Because of skewness, we often transform both CD4 and VL. In our
...
Having constructed the B-spline basis, the SMLEs can be obtained by running the `linear2ph()` function on `data.linear`, as shown in
the code below. Again, the inputs are explained in
...
coefficients and their covariance matrix converged by unsing the `print()` for
